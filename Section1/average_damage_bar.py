...
stacked_chart = alt.Chart(avg_damage_per_category_hour).mark_bar().encode(
    x='time:T',
    y=alt.Y('value:Q', stack='zero', axis=alt.Axis(title='Average Damage Value')),
    color=alt.Color('category:N', legend=alt.Legend(title="Category")),
    opacity=alt.condition(legend_selection, alt.value(1), alt.value(0.2)),  # Use the legend selection for opacity
...
stacked_chart.save('average_damage_bar2.html')
